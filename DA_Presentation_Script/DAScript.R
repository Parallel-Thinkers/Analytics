...
Legend<-paste(sslc$NRC_GENDER_CODE,sslc$URBAN_RURAL)
qplot(sslc$TOTAL_MARKS, col = Legend,fill=Legend,alpha=I(.5) ,geom = "density")
...
numberOfSchoolcode<-unique(sslc$SCHOOL_CODE)
numberOfSchoolPerDistrictCount<-substr(numberOfSchoolcode,1,2)
table(numberOfSchoolPerDistrictCount)
...
numberOfStudentPerDistrictCount<-table(sslc$DIST_CODE)
